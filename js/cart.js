class Cart {
  ...
  async renderCart() {
    ...
    let cartDomSting = `<div class="container">
                <div class="row">
                    <div class="col-5"><strong>Замовлення</strong></div>
  ...
  order(ev) {
    if (this.cartLength() === 0) {
      window.showAlert('Будь ласка, оберіть продукт', false);
      return;
    }    
      ...
      fetch('order', {
        ...
        .then(response => {
          if (response.status === 200) {
            ...
            throw new Error('Не можна відіслати форму');
          }
        })
        .then(responseText => {
          form.reset();
          ...
          this.updateBadge();
          this.renderCart();
          window.showAlert('Дякуємо за замовлення! ' + responseText);
          this.cartContainer.querySelector('.btn-close').click();
        })
        .catch(error => showAlert('Помилка: ' + error, false));
    } else {
      window.showAlert('Будь ласка, коректно заповніть форму', false);
    }
  }
}
